...
class NuPayService {
  ...
  /**
   * Submit a collection / debit order instalment via NuPay.
   */
  async createCollection(payload, tenantId = null) {
    if (process.env.NODE_ENV === 'test' || process.env.NUPAY_MOCK === 'true') {
      const providerRef = `NPM-${Date.now()}`;
      return {
        success: true,
        status: 'SUBMITTED',
        providerReference: providerRef,
        mandateId: payload.mandateId,
        clientReference: payload.clientReference || providerRef
      };
    }
    ...
    const collectionPayload = {
      ...
      clientReference: payload.clientReference || `LOAN-COLL-${Date.now()}`,
      actionDate: payload.actionDate || new Date().toISOString().split('T')[0]
    };

    return this.makeRequest(
      '/wsDebiCheck/add_instalment',
      collectionPayload,
      tenantId,
      (data) => ({
        success: true,
        status: 'SUBMITTED',
        providerReference: data?.referenceNumbers?.instalmentID || payload.clientReference,
        rawResponse: data
      })
    );
  }
  ...
  async getMandateStatus(mandateId, tenantId = null) {
    const report = await this.getMandateReport({ mandateID: mandateId }, tenantId);
    return {
      mandateId,
      status: report?.records?.[0]?.status || 'UNKNOWN',
      rawResponse: report
    };
  }
