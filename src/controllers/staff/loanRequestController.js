...
const getLoanRequestById = asyncHandler(async (req, res) => {
  ...
  if (app.creditAssessment?.verificationHash) {
    ...
    if (calculatedHash !== app.creditAssessment.verificationHash) {
      ...
      console.warn(`⚠️ [Staff Verification Hash Mismatch] Application ${app.applicationId || app._id}: Stored hash does not match calculated hash. Flagging verificationHashValid=false without mutating stored records.`);
    }
  }
